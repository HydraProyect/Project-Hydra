...
Estado: **implementado y cubierto por tests**. Las 34 entidades que heredan de `EntidadConTenant`/`EntidadBase` tienen su filtro global en `CaeManagerDbContext.OnModelCreating`, el `TenantSelladoInterceptor` sella en escritura y rechaza modificaciones cruzadas, y hay un test de aislamiento por agregado (`AislamientoPorAgregadoTests`, uno por cada una de las 34). Lo que sigue pendiente de `ADR-003` son sus condiciones de salida a producción (PostgreSQL, DPA/Términos por tenant), no el mecanismo.
