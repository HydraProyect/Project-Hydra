...
- **Pipeline behaviors** de MediatR, en este orden (de fuera adentro): `LoggingBehavior` (request, duración, tenant, usuario, resultado — nunca el contenido del request), `AccesoDatosPorRequestBehavior` (cada Command/Query recibe su propio `CaeManagerDbContext`, creado bajo demanda vía `IPuertaAccesoDatos`/`IDbContextFactory<CaeManagerDbContext>` en vez de compartir uno solo para todo el circuito de Blazor), `ConcurrenciaBehavior`, `AutorizacionEscrituraBehavior` y `ValidationBehavior` (FluentValidation). **No hay behavior de captura de excepciones de dominio → `Result<T>`**: una `ArgumentException` de entidad sigue llegando cruda a la UI (hallazgo de coherencia de `docs/business/MATURITY_REVIEW.md` § 1, pendiente).
