...
- **Queries**: proyectan directamente a DTOs de lectura con `.Select(...)`, sin pasar por el repositorio de agregados — las queries no tienen invariantes que proteger, solo necesitan ser rápidas. No se usa `AsNoTracking()`: proyectar a un DTO ya no engancha nada al change tracker, así que sería ruido. Sí haría falta en las pocas lecturas que materializan entidades completas. Application define una interfaz de lectura **segregada por feature** (`I{Feature}QueryContext`, ej. `IClientesQueryContext`, `IDocumentosQueryContext`) con una propiedad `IQueryable<T>` por agregado de ese feature — no una única interfaz "god object" con todos los agregados del sistema; `CaeManagerDbContext` las implementa todas en Infrastructure. Un handler inyecta solo la(s) interfaz(es) del/de los feature(s) que realmente consulta — visible en su constructor, no oculto detrás de un contexto omnisciente — incluidos los handlers que agregan varios agregados relacionados por diseño (ej. `ObtenerDocumentosQuery`, que cruza Documentos/Clientes/Empresas/Trabajadores por las FK reales de `Documento`). Application referencia el paquete `Microsoft.EntityFrameworkCore` (la capa de abstracciones, para poder usar `CountAsync`/`ToListAsync`/etc. sobre `IQueryable<T>`) pero **nunca** un proveedor concreto (`Npgsql.EntityFrameworkCore.PostgreSQL`, `...SqlServer`, ...) ni el tipo `CaeManagerDbContext` — eso es exclusivo de Infrastructure. Un test de arquitectura (`tests/CaeManager.Architecture.Tests`) verifica estas fronteras.
