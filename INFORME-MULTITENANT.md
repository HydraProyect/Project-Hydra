...
**Estado**: Análisis y propuesta. **Nada de este documento está implementado ni aprobado.** Corresponde a las Fases 1–2 del proceso acordado (Analizar → Proponer → Debatir → Aprobar → Implementar → Validar). Contiene decisiones que requieren aprobación explícita del propietario del producto antes de pasar a Fase 5 — están marcadas como **[DECISIÓN PENDIENTE]**.
