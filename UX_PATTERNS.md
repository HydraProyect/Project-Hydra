...
En las listas con acordeón (hoy `/centros`; `/empresas` tras el Lote 0-I), la posición donde vivía el checkbox la ocupa el control de expandir/colapsar de la fila (`.boton-expandir-fila`: contraído = flecha a la derecha, expandido = flecha abajo). Al activar selección múltiple el checkbox **se añade a su izquierda sin desplazarlo ni ocultarlo** — ambos conviven. La misma barra ofrece "Expandir todos"/"Colapsar todos", y por eso la expansión la lleva la página (un `HashSet` de Ids) y no el estado interno de cada `SeccionColapsable`: nadie puede decidir por todas desde fuera si cada una guarda el suyo.
