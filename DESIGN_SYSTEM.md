...
| MenuAcciones / ItemMenuAccion (Dropdown) | `src/CaeManager.Web/Components/DesignSystem/MenuAcciones.razor`, `ItemMenuAccion.razor` — H2 (`docs/ux-audit/05-trabajadores-vehiculos.md`): agrupa las 2-3 acciones de fila (antes apiladas verticalmente) tras un disparador "⋯", densidad de una línea por fila en las 8 listas con `QuickGrid`. Cierre al clicar fuera con el mismo truco sin JS interop que `Modal.razor` (superposición a pantalla completa detrás del panel); cada `ItemMenuAccion` recibe el `MenuAcciones` padre por `CascadingValue` y se cierra a sí mismo tras ejecutar su `OnClick`. `Destructivo="true"` en `ItemMenuAccion` da el tono de peligro (Eliminar). **Don't**: usarlo para menos de 2 acciones — un único botón visible es más rápido de escanear que abrir un menú para una sola opción. |
