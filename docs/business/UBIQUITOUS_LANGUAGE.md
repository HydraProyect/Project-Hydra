...
| **Nivel de servicio de Subcontrata** (`Gestionada` / `Supervisada`) | Nivel de servicio contratado sobre una `Subcontrata` (entidad de dominio ya existente): **Gestionada** — se sube y valida su documentación en Hydra (semántica actual) — o **Supervisada** — solo se audita su cumplimiento en las plataformas Inbound del titular mediante Verificaciones Externas registradas, sin cargar nada. El cambio de nivel es una operación de negocio, no un cambio de entidad. Confirmado 2026-08-10 (`DECISION_LOG.md`). Especificación completa en `ADR-005-subcontratas-supervisadas.md` (Aceptado). | `ADR-005-subcontratas-supervisadas.md` |
...
| **Métricas BPO** | Draft | Cuadro de mando ejecutivo para la dirección de una Consultora, eje distinto del dashboard de cumplimiento documental (que mide documentos, no negocio). Fasado propuesto por el propietario del producto (2026-08-10, pendiente de aprobación formal): **Fase 1** — métricas operativas derivadas solo de la actividad del sistema (hilos atendidos por gestor, tiempo de respuesta/SLA, validaciones IA vs. manuales, % cumplimiento por Cliente Delegante); **Fase 2** — métricas financieras con parámetros de cuenta introducidos por la dirección en el tenant de la Consultora (fee mensual por cliente, coste/hora estimado) para calcular margen por cuenta. La agregación por cartera cruza tenants: se hace vía clientes autorizados (patrón Visión de cartera), nunca rompiendo el filtro de tenant. Lista definitiva de KPIs y modelo de estimación de horas sin decidir. | `BUSINESS_MODEL.md` / `UNIT_ECONOMICS.md` |
