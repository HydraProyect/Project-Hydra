...
|---|---|---|---|
| **Acreditación** | Draft | Estado de un Documento frente a una plataforma destino concreta (Pendiente de subir / Subida / Aceptada / Rechazada / No requerida) — bloque MVP1 ya autorizado por el propietario (`docs/ux-audit/PLAN-EJECUCION-UX.md` § Parte 2 (b), `docs/business/DECISION_LOG.md` 2026-08-05). **Nunca "Incidencia"** para lo documental — colisión real con `INBOUND_DOMAIN_GLOSSARY.md` ya detectada. Añadido aquí (Draft, no Approved) al construir el dominio (`AcreditacionDocumentoPlataforma`, Lote 2-C) porque el término en sí no tiene todavía la confirmación explícita de negocio que exige "Reglas de este documento" — solo el bloque que lo usa está autorizado. | `PLAN-EJECUCION-UX.md` |
