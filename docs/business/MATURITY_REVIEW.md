...
29. ✅ **API pública versionada** (`/api/v1` + OpenAPI + API keys por tenant + rate limiting) montada sobre los handlers existentes. — Solo lectura (Clientes/Centros/Trabajadores/Documentos/Asignaciones), OpenAPI nativo de .NET 10 en `/api/v1/openapi.json`, aislamiento multi-tenant heredado sin cambios. Las claves las emite el Administrador de la plataforma, no autoservicio — decisión de alcance para v1, todavía no anunciada como producto.
30. 🟡 Multi-réplica completa (backplane SignalR, leader election, sticky sessions); OpenTelemetry; feature flags por tenant; Licensing/billing. — Multi-réplica hecha (backplane SignalR/Redis, elección de líder vía advisory lock de PostgreSQL, llavero de Data Protection en S3), apagada por defecto. OpenTelemetry, feature flags por tenant y licensing/billing quedaron fuera de alcance a propósito, cada uno para su propia sesión (`ROADMAP.md`, Fase 71).
31. 🟡 Bulk actions, atajos j/k, acciones en el palette, filtros guardados — el kit del usuario experto de 8h/día. — Código mergeado (bulk delete en Clientes/Documentos/Trabajadores, atajos j/k/x/Enter en las 3 rejillas, filtros guardados por usuario, comandos de creación desde el buscador global). Verificación end-to-end en navegador todavía sin cerrar.
32. 🟡 Fronteras de módulo reales (partir `IApplicationDbContext` por feature + tests de dependencia) — el paso honesto hacia el "kernel vs. módulos" que `docs/PLATFORM.md` ya promete. — **Refactor mergeado, PR #60.** Refactor mecánico verificado (137 archivos, `CaeManagerDbContext` implementa las ~30 interfaces segregadas correctamente, sin fugas de copia/pega). Pero los tests de arquitectura nuevos, verificados introduciendo violaciones deliberadas, solo atrapan la reaparición literal del god-interface viejo — no el mismo antipatrón bajo otro nombre, ni un handler cruzando a la interfaz de otra feature sin necesidad, que es justo lo que "fronteras de módulo reales" promete evitar (por eso sigue 🟡, no ✅).
33. 🟡 Primer conector de integración cuando haya proveedor confirmado; SSO federado por tenant. — Conector de Microsoft 365 para Comunicaciones hecho (OAuth delegado por buzón + webhooks entrantes reales vía Graph, `ROADMAP.md` Fase 72). SSO federado por tenant queda fuera, para otra sesión.
