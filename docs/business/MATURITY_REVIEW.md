...
23. ✅ Gate de deploy (Railway solo despliega con CI verde) + smoke test post-deploy. — El gate de "esperar CI" es configuración de GitHub (branch protection), no expresable en código; `DEPLOY.md` lo documenta como paso manual pendiente en vez de afirmar que ya está activo. PR #49.
...
25. 🟡 Pentest externo; Dependabot + escaneo de imagen; `USER` no-root en Dockerfile. — Las tres piezas de código hechas (Dependabot cubre NuGet+Docker+Actions, escaneo Trivy integrado en CI, `USER` no-root aplicado correctamente). Pentest externo sigue pendiente — requiere contratar un proveedor, no es un fix de código.
26. ✅ Ingesta Graph real de Comunicaciones **o congelar el módulo y no venderlo**. — Se optó por congelar: módulo oculto de la navegación, redirección real en vez de una UI que aparente funcionar. PR #49.
