...
### Resolución automática de proveedor desde URL (Canal de gestión, Lote 2-B)
Cuando un dato que el usuario introduciría a mano (aquí, la plataforma CAE de un acceso de gestión documental) se puede **derivar** de otro campo ya obligatorio (la URL de acceso), no se pide como selector aparte por defecto: se resuelve solo, y el selector manual solo aparece cuando la resolución automática no basta. Flujo de `CentroWorkspacePanel.razor` (drawer de canal): al perder el foco el campo "URL de acceso" (`OnBlur` de `CampoTexto`, no en cada tecla), se llama a `IResolucionProveedorPlataformaCaeService.ResolverPorUrlAsync` — 1 candidato se auto-selecciona y se muestra como confirmación de solo lectura (`Badge` + enlace "¿No es correcta? Elegir manualmente" para forzar el selector si la detección se equivocó); 0 o varios candidatos (multi-match real, p. ej. Sabentis/Quirón Prevención comparten dominio) abren automáticamente un `SelectorEntidad` — con **solo** los candidatos si hubo multi-match, con el catálogo completo si no hubo ninguno — sin `PermiteCrear`: el catálogo es global y curado por producto (`docs/MULTITENANCY.md` § 7, mismo criterio que Roles), no se amplía desde un formulario de tenant. Antes de cualquier resolución (formulario recién abierto, URL vacía) no se muestra ningún selector — el único control manual visible de entrada es Plataforma/Correo. Reutilizable para cualquier campo futuro derivable de una URL o de otro dato ya capturado.
