...
Ninguno de los tres modos anteriores cubre a una **consultora externa** (p. ej. Geseme) cuyos usuarios necesitan operar sobre tenants ajenos que no poseen — el caso real que destraba el primer cliente comercial de Hydra. Diseño completo, decisiones cerradas con el usuario, en **`ADR-004-delegacion-consultoras-cae.md`**: la consultora es un `Tenant` sin datos operativos propios (solo identidad de sus usuarios); una tabla `DelegacionTenant` + `AsignacionUsuarioDelegado` autoriza a usuarios concretos a resolver como activo un tenant ajeno; el filtro global y el interceptor de sellado (§ 4) **no cambian** — la delegación decide solo de dónde sale el `TenantId` de la sesión, nunca cómo se aplica. No implementado — léelo antes de tocar `ITenantActual`, el flujo de login, o cualquier cosa relacionada con "un usuario ve más de un tenant".
