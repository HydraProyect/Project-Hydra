...
| `IDocumentAIProvider` + Factory + capacidades | ✅ **Completa (Fase 2, 2026-07-24; segundo proveedor real en Fase 46, 2026-07-25)** — es la generalización que `docs/PLATFORM.md` § 4 ya preveía, no una capa añadida "por si acaso". `DocumentAIProviderFactory` (Application) + `AnthropicDocumentAIProvider`/`GeminiDocumentAIProvider` (Infrastructure) registrados; `MistralOcrDocumentAIProvider` sigue sin construir (sin clave provisionada). |
...
**Nota sobre "Mistral OCR 4" (verificado julio 2026, no implementado)**: Mistral lanzó el 23/06/2026 una versión de su OCR con bounding boxes, tipo de bloque, confianza por palabra/página, markdown estructurado, 170 idiomas y self-hosting — un salto real respecto al "OCR auxiliar" que este documento asumía originalmente, no una mejora incremental. Precio: $4/1.000 páginas (OCR), $5/1.000 (Document AI con salida estructurada por esquema), con 50% de descuento en Batch API. El confidence score por bloque/página encaja directamente con lo que ya construyó Hydra en la Fase 45 (`AprobacionDocumento`, automática vs manual según confianza) — candidato serio a proveedor principal de estructuración para documentos escaneados/con tablas, no solo a "OCR auxiliar" como se planteó en § 2.3 originalmente. **Recomendación antes de fijar el reparto de capacidades Mistral/Gemini**: benchmarkear con un lote real de documentos de Hydra (pólizas, recibos, certificados, documentos PRL, documentos de obra, contratos — escaneados, con tablas, con muchas páginas) en vez de asumir "PDF digital → Gemini / escaneado → Mistral" por decisión fija — ver `MistralOcrDocumentAIProvider`, todavía sin construir (sin clave provisionada).
