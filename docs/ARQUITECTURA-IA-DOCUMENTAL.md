...
**Alcance real de esta implementación, más acotado que la nota anterior**: solo se implementó `ExtraerTextoAsync` (endpoint OCR base, `POST /v1/ocr`, verificado cruzando varias fuentes) — declara únicamente `CapacidadesProveedorIa.OcrImagenAEscaneado`, respetando el reparto original (Mistral = OCR, Gemini = estructuración) hasta que exista un benchmark real. **`ExtraerEstructuradoAsync` NO se implementó** (la función "Document AI" de Mistral, extracción estructurada por esquema JSON) — varias páginas oficiales de Mistral (`mistral.ai/news/ocr-4`, `docs.mistral.ai/api/endpoint/ocr`) devolvieron 403 al intentar verificarlas directamente durante la implementación (confirmado que era un bloqueo de proxy del entorno de esta sesión, no un bloqueo del sitio — hasta `https://example.com` devolvía el mismo 403 en ese momento), así que no se adivinó el contrato HTTP exacto de `document_annotation_format`. El método existe (la interfaz lo exige) pero devuelve un fallo controlado explícito (`DocumentAIProvider.NoSoportado`) — se completa cuando se pueda verificar el formato real. **Recomendación antes de fijar el reparto de capacidades Mistral/Gemini definitivo**: benchmarkear con un lote real de documentos de Hydra (pólizas, recibos, certificados, documentos PRL, documentos de obra, contratos — escaneados, con tablas, con muchas páginas) en vez de asumir "PDF digital → Gemini / escaneado → Mistral" por decisión fija.
