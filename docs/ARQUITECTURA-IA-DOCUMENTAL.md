...
**Estado**: Propuesta de arquitectura, decisiones cerradas con el usuario el 2026-07-24. **Las 5 fases de § 5 están completas** (clasificación local, `IDocumentAIProvider`/Factory/adaptador Anthropic, el router con los 4 casos + reintento inteligente ya conectado al flujo real de `VerificacionIaDocumentoService`, caché por SHA256 + auditoría, y localización de páginas relevantes en documentos digitales grandes) — ver § 3 y `ROADMAP.md`. **Actualización (Fase 46, 2026-07-25)**: `GeminiDocumentAIProvider` real ya construido y registrado (clave provisionada en Railway) — configurado sobre `gemini-3.5-flash`, no `gemini-2.5-flash` (que Google retira el 16/10/2026, ver § 4.1). **Actualización (Fase 47, 2026-07-25)**: `MistralOcrDocumentAIProvider` real construido y registrado (clave provisionada) — solo la capacidad de OCR (`ExtraerTextoAsync`); la extracción estructurada ("Document AI") no se activa porque `ExtraerEstructuradoAsync` recibe texto pre-extraído, incompatible con el modo de operación de Mistral Document AI sobre documento en bruto (ver § 4.1). Queda pendiente, antes de fijar qué proveedor hace qué de forma definitiva: un benchmark real con documentos de Hydra en vez de la asunción original "PDF digital → Gemini / escaneado → Mistral" (Mistral lanzó en junio de 2026 "OCR 4", con confianza por bloque y estructura espacial que lo convierten en candidato serio también para estructuración, no solo OCR — ver § 4.1). Sigue la disciplina de `CLAUDE.md`: Dominio → Arquitectura → Plataforma → Implementación — este documento cubre las tres primeras.
...
| Localización de páginas relevantes (pólizas de 200+ páginas) | ✅ **Completa (Fase 43, 2026-07-24)** — ver § 4.3. `ILocalizadorPaginasRelevantesService`, búsqueda local de palabras clave (sin IA), integrado en el router para el Caso 1 (digital). |
...
| `IDocumentAIProvider` + Factory + capacidades | ✅ **Completa (Fase 2, 2026-07-24; segundo y tercer proveedor real en Fases 46-47, 2026-07-25)** — es la generalización que `docs/PLATFORM.md` § 4 ya preveía, no una capa añadida "por si acaso". `DocumentAIProviderFactory` (Application) + `AnthropicDocumentAIProvider`/`GeminiDocumentAIProvider`/`MistralOcrDocumentAIProvider` (Infrastructure) registrados — los tres reales, con clave provisionada. |
...
**Actualización (Fase 46, 2026-07-25)**: segundo proveedor real, `GeminiDocumentAIProvider`, añadido en cuanto hubo clave real de Gemini que probar — mismo criterio que arriba. Declara solo `ExtraccionEstructurada` (implementa también `ExtraerTextoAsync`, porque Gemini lee PDF/imagen de forma nativa igual que Claude, pero no se activa como capacidad hasta que haga falta comparar proveedores de OCR — cambiarlo es una línea, no un adaptador nuevo). **Modelo configurado: `gemini-3.5-flash`, no `gemini-2.5-flash`** — verificado en julio de 2026 que Google anuncia el retiro de 2.5 Flash el 16/10/2026 (~3 meses después de esta implementación), con 3.5 Flash como ruta de migración recomendada sin fecha de retirada anunciada. Construir hoy sobre un modelo con fecha de caducidad ya conocida habría significado una segunda migración a los pocos meses — se evita desde el principio. Precio de referencia usado en `GeminiOptions` (solo para auditoría, § 4.2): $1,50/M tokens entrada, $9,00/M salida (3.5 Flash) — más caro que 2.5 Flash, pero es el coste real vigente, no uno ya obsoleto.

**Actualización (Fase 47, 2026-07-25)**: tercer proveedor real, `MistralOcrDocumentAIProvider`, añadido en cuanto hubo clave real de Mistral que probar. "Mistral OCR 4" (lanzado 23/06/2026, verificado por búsqueda antes de construir nada) devuelve bounding boxes, tipo de bloque, confianza por palabra/página, markdown estructurado, 170 idiomas y self-hosting — un salto real respecto al "OCR auxiliar" que este documento asumía originalmente, no una mejora incremental. Precio: $4/1.000 páginas (OCR), $5/1.000 (Document AI con salida estructurada por esquema), con 50% de descuento en Batch API. El confidence score por bloque/página encaja directamente con lo que ya construyó Hydra en la Fase 45 (`AprobacionDocumento`, automática vs manual según confianza).

**Alcance real de esta implementación**: solo se implementó `ExtraerTextoAsync` (endpoint OCR base, `POST /v1/ocr`, contrato HTTP verificado por el usuario — ver nota de Fase 47 en `ROADMAP.md`) — declara únicamente `CapacidadesProveedorIa.OcrImagenAEscaneado`, respetando el reparto original (Mistral = OCR, Gemini = estructuración) hasta que exista un benchmark real. **`ExtraerEstructuradoAsync` NO se implementó** (la función "Document AI" de Mistral, parámetro `document_annotation_format`) — el contrato HTTP está verificado, pero la incompatibilidad es de firma: `ExtraerEstructuradoAsync(string texto, ...)` recibe texto pre-extraído, mientras que Mistral Document AI opera sobre el documento original en bruto (`document_url`/`image_url`/`file_id`). Se completaría si se añade un método que reciba el documento en bruto directamente. El método existe (la interfaz lo exige) pero devuelve un fallo controlado explícito (`DocumentAIProvider.NoSoportado`). **La forma correcta del `document` para imágenes** (`ImageURLChunk`) es un objeto anidado `{"type":"image_url","image_url":{"type":"image_url","url":"data:..."}}`, no una cadena directa — diferente del caso PDF (`DocumentURLChunk`: `"document_url"` sí es una cadena plana). **Recomendación antes de fijar el reparto de capacidades Mistral/Gemini definitivo**: benchmarkear con un lote real de documentos de Hydra (pólizas, recibos, certificados, documentos PRL, documentos de obra, contratos — escaneados, con tablas, con muchas páginas) en vez de asumir "PDF digital → Gemini / escaneado → Mistral" por decisión fija.

**Orden de proveedores en el router** (`InfrastructureServiceCollectionExtensions`, importa porque `DocumentAIRouterService` no reintenta en el paso de OCR, solo en el de estructuración — Fase 41): Mistral se registra antes que Anthropic, así que es el proveedor OCR usado en cuanto tenga clave (Anthropic pasa a ser el respaldo de OCR); para estructuración, Anthropic sigue siendo el primario y Gemini el candidato de reintento — cambiar quién es primario en estructuración es una decisión de benchmark, no un efecto colateral de añadir una clave nueva.
...
Confirmado inicialmente: la localización de páginas relevantes en documentos grandes (pólizas de cientos de páginas) y el Document Graph/Expedientes quedan fuera de la primera entrega, como una fase separada posterior — no bloqueaban la construcción del router de proveedor.

**Actualización (Fase 43, 2026-07-24)**: la localización de páginas se implementó como fase separada, tal como estaba prevista. `IExtractorTextoDigitalService.ExtraerTextoPorPagina` ahora devuelve el texto **por página** (antes, un único string concatenado) — cambio necesario para que el router pueda decidir qué páginas enviar antes de unir el texto. `ILocalizadorPaginasRelevantesService` (Application/DocumentosIa) es una búsqueda local de palabras clave orientada a documentos CAE/seguros en español (número de póliza, tomador, fechas, firma...), sin IA — mismo criterio que el clasificador (§ 3): no vale la pena pagar un proveedor de IA solo para decidir qué páginas importan. Siempre incluye la primera y la última página aunque no contengan ninguna palabra clave (portada/cierre).

El router (`DocumentAIRouterService.ObtenerTextoAsync`) solo aplica la localización al Caso 1 (documento digital) y solo por encima de un umbral (`UmbralPaginasParaLocalizar = 15`) — documentos pequeños se mandan completos, sin coste de localización. Cuando se descartan páginas, queda constancia en `AuditoriaExtraccionIa.Incidencias` (p. ej. "Documento grande (280 páginas): se seleccionaron 9 páginas relevantes antes de enviar a IA") — visible, pero **no bloquea nada**: es solo trazabilidad de qué se decidió mandar a IA y por qué. El Document Graph/Expedientes sigue fuera de alcance.
...
5. ✅ Localización de páginas relevantes en documentos digitales grandes (Fase 43, 2026-07-24) — ver § 4.3.
