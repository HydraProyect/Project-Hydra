...
| **Notifications** | Ya sigue el patrón correcto para dos canales — ver § 4. WhatsApp (Fase 84) no cuenta como tercero: es canal de negocio (Comunicaciones), no notificación saliente — ver § 4 | `IEmailService`/`GraphEmailService`, `NotificacionUsuario` (interno) |
...
- **Notification Provider**: `IEmailService` (email) y `NotificacionUsuario` (interno) ya son dos canales desacoplados. **WhatsApp ya es real (Fase 84)** pero no se construyó como tercer canal de esta abstracción — es un canal bidireccional (webhook + envío + chat en vivo) del módulo de negocio Comunicaciones (`ConversacionCorreo.Canal`, ver `ARQUITECTURA-INTEGRACIONES.md` § 12.7 y `DOMAIN.md`), no una notificación saliente genérica. SMS/Teams/Slack siguen sin implementar; si llegan como notificación saliente pura sí encajarían aquí.
