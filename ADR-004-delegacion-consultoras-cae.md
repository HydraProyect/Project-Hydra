...
- **11.5 — No existen tenants híbridos** (añadida 2026-08-10, origen distinto al resto de § 11: decisión del propietario del producto registrada en `docs/business/DECISION_LOG.md`). Un `Tenant` es Cliente Directo/Delegante (tiene datos operativos propios) **o** Consultora (§ 5.1: cartera de delegaciones, cero filas de `Empresa`/`Centro`/`Trabajador`/`Documento`), nunca las dos cosas. Si una organización necesita ambos papeles, se modela como **dos tenants** vinculados por una `DelegacionTenant` entre ellos, y su gestor opera su propia empresa con el mismo selector de Delegated Workspace que usa con el resto de clientes. Consecuencia para el código: **no se añaden excepciones por tenant** (`if (tenantId == 1)`, banderas de "tenant legacy híbrido" o similares) en dominio, filtros ni autorización — el motor no conoce casos especiales.
...
2. ~~**Quién puede crear/desactivar una `DelegacionTenant`**~~ — **Resuelto parcialmente (2026-08-01), v1 mínima (hallazgo P0-7 de `docs/business/MATURITY_REVIEW.md`)**: `CrearClienteDeleganteCommand` da de alta el Cliente Delegante, la `DelegacionTenant` activa y el primer `AsignacionOperadorDelegado` en una sola operación, autorizado únicamente si el tenant de origen de quien ejecuta tiene `EsPlataforma = true` (mismo criterio que `AbrirAccesoSoporteCommand`) — "solo Administrador de plataforma" era la decisión mínima que el propio hallazgo dejaba como aceptable para v1. **Sigue sin flujo de autoservicio**: si el alta recae en el administrador del Cliente Directo, en la Consultora, o en ambos con distinto alcance, sigue abierto y tiene implicaciones comerciales (¿es un flujo de venta autoservicio o gestionado por Hydra?) — coordinar con `docs/business/BUSINESS_ARCHITECTURE.md` cuando se desarrolle. Desactivar ya estaba resuelto antes de este hallazgo (`DesactivarDelegacionTenantCommand`, pantalla `/delegaciones`). **Cerrado (2026-08-10)** — decisión del propietario del producto, registrada en `docs/business/DECISION_LOG.md`: la **vinculación** entre dos tenants que ya existen es **autoservicio descentralizado**, sin intervención del Administrador de plataforma, con dos flujos simétricos (el Cliente Delegante invita a la Consultora; o la Consultora solicita y el Cliente Delegante aprueba) y una única autoridad: solo un usuario con rol `Administrador` **en el tenant del Cliente Delegante** aprueba, modifica o revoca la `DelegacionTenant`. Coherente con § 11.1 (Hydra nunca inicia una delegación) y § 11.3 (Hydra aplica, no arbitra). **Alcance de lo decidido**: es la política, **no está implementada** — hoy el único camino de alta sigue siendo `CrearClienteDeleganteCommand` (solo plataforma), que además **crea el tenant del Cliente Delegante desde cero**; ese caso ("el Cliente Delegante todavía no existe en Hydra") permanece en la plataforma y no lo cubre el autoservicio. Implicaciones comerciales del flujo autoservicio: coordinar con `docs/business/BUSINESS_ARCHITECTURE.md` al implementarlo.
...
6. ~~**Migración de datos existentes**~~ — **Resuelto (2026-07-30), confirmado con el usuario**: los datos operativos que tenía el tenant #1 eran datos de prueba, no datos reales de producción — se eliminaron y se restructuró el seed. El tenant #1 queda como Consultora sin datos operativos propios (§ 5.1) y un tenant nuevo ("Ibertec S.A. (Cliente Delegante demo)", creado por `DelegacionDemoSeeder`) concentra todos los datos CAE de prueba, con una `DelegacionTenant` activa entre ambos. Esto resolvió el caso demo. **Cerrado del todo (2026-08-10)**, con la reserva que quedaba ("si en el futuro hay un tenant #1 con datos operativos **reales**"): la respuesta es § 11.5 — **no se soportan tenants híbridos**. Un tenant que acumule ambos papeles se **escinde** en dos (Cliente Directo con los datos operativos + Consultora con la cartera, unidos por una `DelegacionTenant`) mediante migración de datos puntual; nunca mediante excepciones de código. Nota sobre el estado real: la escisión del tenant #1 **ya ocurrió** en la práctica el 2026-07-30 por la vía de borrar datos de prueba, así que no hay script de migración pendiente — lo que se cierra aquí es la regla para cualquier caso futuro.
