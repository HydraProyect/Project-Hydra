...
- **Fecha de RNT/RLC**: el documento no imprime una fecha de emisión propia — es **el día 1 del mes del "Periodo de liquidación"** (periodo 06/2026 → fecha emisión 01/06/2026). La vigencia la calcula `CalculadoraEstadoDocumento` a partir de `TipoDocumento.VigenciaMeses`, sin tocar el parser — **3 meses** (ya así en el seed desde PR-2, confirmado por el usuario: el periodo 06/2026 debe quedar vigente hasta finales de agosto — 01/06 + 3 meses). Derivación nueva y genérica en `ParserDocumentoOficialBase` (`FechaEmisionEsPrimerDiaDelPeriodo`), no un caso especial de RLC/RNT.
...
### PR-7 (pendiente, requiere muestra real): cotejo RLC ↔ recibo de pago en archivos combinados

Confirmado por el usuario: el tipo "RLC/TC1 + Recibo de pago" puede traer **varias liquidaciones en un único archivo**, cada una con su propio recibo — sin entidad bancaria fija (no se puede anclar por banco). El cotejo exigido entre cada RLC y su recibo: **razón social, importe (monto) y número/código de liquidación deben coincidir** (el periodo no es un campo bloqueante en sí, viene implícito en el número de liquidación).

**Implementado ya, sin necesidad de muestra**: `ParserRlc.AdvertirSiMultiplesPeriodos` detecta si el archivo contiene más de un periodo de liquidación **distinto** (valores únicos de `RegexPeriodoPorForma`, no repeticiones de cabecera/pie del mismo periodo — verificado con test) y, si es así, manda el documento entero a revisión con motivo explícito ("el archivo contiene varias liquidaciones... sube un documento por periodo") — nunca extrae ni coteja a ciegas contra el primer periodo que matchee. Solo activo en `ParserRlc`; `ParserRnt` no lo activa (no se ha confirmado que el RNT bundlee periodos).

**Pendiente de una muestra real del tipo combinado** (con al menos 2 liquidaciones+recibos en el mismo archivo, tratada con la misma confidencialidad que las muestras de PR-6 — solo en sesión, nunca al repo): diseñar cómo se repiten los bloques (¿RLC y su recibo van seguidos, o todos los RLC primero y los recibos después?), extraer instancias múltiples de razón social/importe/número de liquidación por bloque y emparejarlas, y solo entonces auto-validar por par. Hasta esa muestra, el fail-safe actual (revisión en vez de emparejar a ciegas) es la garantía correcta — cero riesgo de auto-validar un par mal emparejado, a costa de mandar siempre a revisión los archivos combinados.
...
- **Vía a investigar primero: REASS** (Registro Electrónico de Apoderamientos de la Seguridad Social, Orden ISM/189/2021, vigente 2026 — sede.seg-social.gob.es). Más preciso que "autorización RED genérica": el cliente apodera específicamente a Hydra para trámites concretos (hay subtipos de apoderamiento limitados a consulta/recepción de duplicados, sin ceder gestión laboral completa), autenticado con certificado propio de Hydra (persona jurídica). Encaja como conector de `ARQUITECTURA-INTEGRACIONES.md` (capacidad `VerificacionDocumental`), con su circuito de credenciales por tenant. **Pendiente de confirmar con la TGSS**: qué trámites concretos (verificación CEA, duplicado RLC/RNT, ITA) están realmente cubiertos por REASS y con qué interfaz técnica (no confundir con **FIER**, servicio real de RED Online pero para el fichero de prestaciones INSS por trabajador — invalidez, jubilación, maternidad —, sin relación con RLC/RNT/ITA/certificados).
...
- **Vía descartada: custodia de certificados de terceros + RPA** contra el portal simulando al usuario. Alguna plataforma del sector lo hace, pero implica custodiar credenciales/certificados ajenos y suplantar la sesión del cliente ante un portal transaccional — riesgo legal y de seguridad que no compensa frente a REASS (apoderamiento explícito y acotado) ni frente al valor real que aporta (verificar un dato, no operar como si fuéramos el cliente).
