...
1. Habilita `ROW LEVEL SECURITY` con `FORCE` en las 40 tablas que llevan `TenantId` que existían en ese momento (las mismas de `CaeManagerDbContext.OnModelCreating`). Tres migraciones posteriores ampliaron la lista según se creaban tablas nuevas con `TenantId` — `TrabajosAnalisisDocumento` (mismo día, `20260801130000_AgregarTrabajosAnalisisDocumento`), `ClavesApi` (P3-29, `20260802165602_HabilitarRlsClavesApi`) y las 4 tablas del conector Microsoft 365 (P3-33, `20260802191958_HabilitarRlsIntegraciones`) — 46 tablas en total a día de hoy. **`PoliticasRlsCubrenModeloTests`** (`tests/CaeManager.IntegrationTests/Tenants/`) compara `pg_policies` contra el modelo de EF en cada build de CI para que la próxima tabla nueva que se quede fuera falle el test en vez de pasar desapercibida — la lista fija en cada migración sigue siendo manual (toda tabla nueva con `TenantId` necesita su propia migración `HabilitarRlsX`, ver `HabilitarRlsIntegraciones` como plantilla), pero ahora hay una red que lo detecta.
