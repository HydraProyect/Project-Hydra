...
- La política sigue **apagada por defecto** (`RetencionDatos:Activa = false`): mientras no se active no se detecta ni se purga nada. Ver [Issue #11](https://github.com/christopherjp1-jpg/Project-Hydra/issues/11).
- **Las filas soft-deleted (`EstaEliminado`) SÍ entran en detección/anonimización** — decidido por el usuario (2026-08-01, cierre de P0-3 de `docs/business/MATURITY_REVIEW.md`). Antes, el filtro global (`!EstaEliminado`) las ocultaba también de `DeteccionPurgaService`/`EjecucionPurgaService`, así que un trabajador dado de baja **y** eliminado lógicamente —el caso más claro de "ya no hay relación"— nunca entraba en el ciclo de retención: sus datos, incluidos los de salud, persistían indefinidamente fuera de cualquier plazo. Ambos servicios usan ahora `IgnoreQueryFilters()` con el `TenantId` reaplicado explícitamente en la propia query (revisión deliberada, no un bypass genérico): ven lo soft-deleted, sin dejar de acotar al tenant actual. No cambia nada del resto de la invariante — sigue sin haber camino a "ejecutada" sin autorización expresa con fecha.
...
|---|---|---|
| Railway | Aloja la aplicación y la base de datos completa (todo lo de la tabla del punto 1) | Región UE (`europe-west4`, Ámsterdam) para ambos servicios. **App y Postgres confirmados en `europe-west4` (2026-08-01)** — ver `ROADMAP.md`, "Backlog — Migración a PostgreSQL". Todo el tratamiento de datos personales queda dentro de la UE |
...
| Anthropic / Mistral / Gemini (activos: lectura y verificación de Documentos, asistente, detección de trabajadores) | Contenido de PDFs, incluidos reconocimientos médicos cuando `LecturaIaActiva`/`VerificacionIaActiva` alcanzan ese tipo — **incluye datos de salud** | Ver política de privacidad/DPA de cada proveedor antes de tratar datos reales |

Un DPA formal con cada uno de estos subencargados es el [Issue #13](https://github.com/christopherjp1-jpg/Project-Hydra/issues/13). Con la vía SaaS vigente (`ADR-003-saas-multitenant.md`, que supersede la pausa de `ADR-002-single-tenant.md`), este DPA es condición de salida a producción antes de operar con un tenant real (P0-4 de `docs/business/MATURITY_REVIEW.md`) y además tiene que declarar el acceso de soporte cross-tenant.

**Mitigación técnica mientras el DPA no exista** (P0-4): la detección automática de trabajador/tipo al subir un Documento (`DetectarCamposDocumentoQuery`) enviaba el PDF completo al proveedor de IA **antes** de conocer el tipo de documento, sin que ningún interruptor de `TipoDocumento` la protegiera — así que un reconocimiento médico también pasaba por ahí. Queda apagada por defecto (`DeteccionPreviaDocumento:Activa = false`, ver `DEPLOY.md`): el alta manual de Documento sigue funcionando, solo sin la sugerencia automática. Actívese explícitamente solo cuando el DPA cubra este tratamiento.
