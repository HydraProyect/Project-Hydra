...
**Ojo si además activas `DataProtection__S3__Activo` (P3-30, llavero compartido para multi-réplica, ver `DEPLOY.md`)**: `BackupHostedService` sigue zipeando siempre el directorio *local* `dataprotection-keys/`, nunca el bucket de S3. En cuanto `DataProtection__S3__Activo=true` entra en vigor, la app deja de leer/escribir claves ahí — el directorio local queda vacío (o con las claves viejas, ya sin uso) y el backup automático de claves pasa a subir un `.zip` vacío en silencio, sin ningún error ni aviso en el log. Antes de activarlo en un despliegue que ya tiene claves reales en el volumen: migra `dataprotection-keys/` a S3 a mano (mismo prefijo) para no dejar irrecuperable ninguna `CredencialAccesoEmpresa`/`CredencialAccesoSubcontrata` cifrada con las claves viejas (ver más arriba). No hay una migración automática implementada a propósito — es un cambio de infraestructura que se hace una vez, no algo para automatizar por si acaso (YAGNI).
