...
Aislamiento implementado y auditado con tests "tenant A no ve a tenant B" por agregado; índices únicos compuestos; almacenamiento de archivos particionado por tenant; migración a PostgreSQL (SQLite no sostiene escritura concurrente multi-organización); DPA y Términos de Uso por tenant (con revisión legal, nunca implementación unilateral — regla de `CLAUDE.md`); sin self-signup ni billing antes de todo lo anterior (regla de `ADR-001`).
