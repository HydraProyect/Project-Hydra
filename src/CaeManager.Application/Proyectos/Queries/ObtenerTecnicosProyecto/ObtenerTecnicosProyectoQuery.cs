...
public class ObtenerTecnicosProyectoQueryHandler(IApplicationDbContext dbContext)
    : IRequestHandler<ObtenerTecnicosProyectoQuery, IReadOnlyList<TecnicoProyectoDto>>
{
    ...
    {
        var consulta =
            from proyectoTecnico in dbContext.ProyectosTecnicos
            where proyectoTecnico.ProyectoId == request.ProyectoId
            join trabajador in dbContext.Trabajadores on proyectoTecnico.TrabajadorId equals trabajador.Id
