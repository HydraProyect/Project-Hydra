...
public partial class ImportarClientes : ComponentBase
{
    ...
    [
        ...
        "Comprobando Clientes y Centros existentes…",
    ...
    private async Task ManejarArchivoSeleccionadoAsync(InputFileChangeEventArgs e)
    {
        ...
        try
        {
            ...
            _plan = await Mediator.Send(new AnalizarPlantillaClientesQuery(memoria.ToArray()));
        }
        catch (Exception)
        {
            _mensajeError = "No pudimos leer este archivo. Comprueba que sea la plantilla de clientes descargada desde esta página.";
        }
        finally
        {
            _analizando = false;
