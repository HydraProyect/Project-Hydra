...
public partial class Retencion : CaeManager.Web.Components.PaginaIntegrableConfiguracionBase
{
    ...
    /// <summary>
    /// Junta las dos señales, independientes entre sí: la resuelta es una
    /// solicitud ya atendida, la enfocada es donde está el cursor de teclado.
    /// </summary>
    private string ClaseFila(Guid id, bool resuelta)
    {
        var enfocada = _idEnfocado == id;
        return (resuelta, enfocada) switch
        {
            (true, true) => "fila-resuelta fila-enfocada",
            (true, false) => "fila-resuelta",
    ...
    /// <summary>
    /// Solo "j"/"k": recorren las solicitudes de purga. Aquí NO hay "Enter" ni
    /// "x" y es deliberado — las acciones de la fila son avisar, programar o
    /// purgar: escrituras, una de ellas irreversible. Ninguna puede colgar de
    /// una tecla que en el resto del producto significa "abrir".
    /// </summary>
    private Task ManejarAtajoAsync(string tecla)
    {
        var visibles = _solicitudes;
